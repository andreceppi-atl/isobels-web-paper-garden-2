...
function overlay(region) {
  ...
  const lines = solids.map((solid, index) => {
    ...
    const role = solid.collisionRole ? ` ${solid.collisionRole}` : '';
    const body = solid.collisionRole ? `<rect class="body${role}" x="${x}" y="${y}" width="${w}" height="${solid.h / scale}"/>` : '';
    const label = solid.collisionRole ? '' : `<text x="${x + 3}" y="${Math.max(12, y - 6)}">${String(index + 1).padStart(2, '0')} ${solid.feature}</text>`;
    return `<g>${body}<path d="M ${x} ${y} H ${x + w}"/><path class="tick" d="M ${x} ${y - 4} V ${y + 4} M ${x + w} ${y - 4} V ${y + 4}"/>${label}</g>`;
  }).join('');
  ...
  return Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}">
    <style>path{fill:none;stroke:#00a86b;stroke-width:2;shape-rendering:crispEdges}.tick{stroke-width:1}.body{fill:#00a86b;fill-opacity:.12;stroke:#00a86b;stroke-width:1}.body.bud{fill:#f023a5;stroke:#c00070}.body.house{fill:#ff9f1c;stroke:#b86000}.sign{fill:#1c65f4;fill-opacity:.1;stroke:#1c65f4;stroke-width:2}text{font:7px monospace;fill:#004d32;paint-order:stroke;stroke:#fff;stroke-width:2px;stroke-linejoin:round}.sign-label{fill:#1647a5}</style>
